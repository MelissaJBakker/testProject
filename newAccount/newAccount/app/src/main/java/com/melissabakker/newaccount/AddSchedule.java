...
public class AddSchedule extends AppCompatActivity {
        ...
        btnSched.setOnClickListener(new View.OnClickListener() {
                                        ...
                                        public void onClick(View view) {
                                            try {
                                                ...
                                                Toast.makeText(getApplicationContext(), "Error! Invalid fields.", Toast.LENGTH_LONG);
                                                e.printStackTrace();
                                            }
                                        }
                                    }
    ...
    public boolean checkTime(ArrayList<EditText> timesArray) throws ParseException {
        ...
        ArrayList<SwitchCompat> switches = new ArrayList<>();
        ...
        for (int i = 0; i < 7; i++) {
            if (switches.get(i).isChecked()) {
                String t1 = timesArray.get(i * 2).getText().toString().trim();
                String t2 = timesArray.get((i * 2) + 1).getText().toString().trim();
                Date time1 = sdf.parse(t1);
                Date time2 = sdf.parse(t2);

                if (t1.isEmpty() || t2.isEmpty()) {
                    Toast.makeText(getApplicationContext(), "Error! Missing fields.", Toast.LENGTH_SHORT).show();
                    return false;
                }
